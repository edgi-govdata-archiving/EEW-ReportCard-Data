...
u <- import( 'Region' )
region <- u$Region(db='region_cds.db', type='State', state=state,
                   programs=c('CAA', 'CWA', 'RCRA'),
...
cwa_insp_years = create_df(db='region_cds.db', target_year=r.target_year_int,
               region_type='State',data_type='inspections',
               y_field='Count',program='CWA',states=[r.this_state,])
cwa_viol_years = create_df(db='region_cds.db',
               target_year=r.target_year_int,region_type='State',
               data_type='violations',
               y_field='Count',program='CWA',states=[r.this_state,])
cwa_enf_count_years = create_df(db='region_cds.db',
               target_year=r.target_year_int,region_type='State',
               data_type='enforcements',
               y_field='Count',program='CWA',states=[r.this_state,])
cwa_enf_count_years = cwa_enf_count_years.drop('Amount', axis=1)
cwa_enf_dollar_years = create_df(db='region_cds.db',
               target_year=r.target_year_int,region_type='State',
               data_type='enforcements',
...
actionchange <- data.frame("BushObamaTrumpAverage" =mean(enforcement[1:20,3]),
                             "Trump Average" =mean(enforcement[21:23,3]))
...
if ( nrows_df > 0 ) {
  ...
  #adjust for inflation
  CAAenforcement <- merge(CAAenforcement, inflation, by.x='Year', all.x=TRUE)
...
if ( nrows_df > 0 ) {
  ...
  #adjust for inflation
  CWAenforcement <- merge(CWAenforcement, inflation, by.x='Year', all.x=TRUE)
...
if ( nrows_df > 0 ) {
  ...
  #adjust for inflation
  RCRAenforcement <- merge(RCRAenforcement, inflation, by.x='Year', all.x=TRUE)
