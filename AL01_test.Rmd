...
CD_ord <- '1st'
state <- 'AL'
long_state <- "Alabama's"
cd_state <- paste(state, CD, sep="")
...
##### `r long_state` `r CD_ord` District; seat held by `r leg_info$name` since `r leg_info$since_year` 
 <div class="container">
...
CAArecurring[2,1] <- as.character(paste(state,CD))
CAArecurring[2,4] <- as.character(paste(state,CD))
...
RCRArecurring[2,1] <- as.character(paste(state,CD))
RCRArecurring[2,4] <- as.character(paste(state,CD))
...
```
# Highlights for `r long_state` `r CD_ord` District
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here(output_loc), scale = 3, width=7, height=4, units=c("cm"))
}
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here(output_loc), scale = 3, width=7, height=4, units=c("cm"))
}
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here(output_loc), scale = 3, width=7, height=4, units=c("cm"))
}

CAAfacilities / CWAfacilities / RCRAfacilities 

img_start <- "'<br /><a ><img src=\""
img_part2 <- "\" style=\"float: left; width: 55%; height: 60%; margin-right: 5%\" ></a>    <font size=\"3\">**ECHO reports for facilities:**</font><font size=\"1.5\"><br />'"

CAA_fac_src <- paste(output_loc,"CAAfacilities.png",sep="")
CWA_fac_src <- paste(output_loc,"CWAfacilities.png",sep="")
RCRA_fac_src <- paste(output_loc,"RCRAfacilities.png",sep="")

CAA_img <- paste(img_start,CAA_fac_src,img_part2,sep="")
CWA_img <- paste(img_start,CWA_fac_src,img_part2,sep="")
RCRA_img <- paste(img_start,RCRA_fac_src,img_part2,sep="")

#    "<br /><a ><img src=\""`r fac_src`"\" style=\"float: left; width: 55%; 
