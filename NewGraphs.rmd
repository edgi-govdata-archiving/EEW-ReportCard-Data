...
usa_events = usa_region.get_events( 'inspections', program, 2021 )
usa_events['USA'] = usa_events['Count']/usa_num_facs
state_events_dict = {}
for state in states:
    state_region = Region( type='State', state=state, 
                programs=[program,] )
    state_num_facs = state_region.get_active_facilities( program )
    state_events = state_region.get_events( 'inspections', program, 2021 )
    state_events[ state ] = state_events['Count']/state_num_facs
    state_events_dict[ state ] = state_events
local_region = Region( type='inspections', state=states[0], value='06',
                     programs=[program,])
local_num_facs = local_region.get_active_facilities( program )
local_events = local_region.get_events( 'inspections', program, 2021 )
local_events[ '06' ] = local_events['Count']/local_num_facs
df_events = usa_events.drop( 'Count', 1 )
for state_name,state_events in state_events_dict.items():
    df_events = df_events.merge( state_events[['Year',state_name]] )
if ( region_type != 'State' ):
    df_events = df_events.merge( local_events[['Year','06']])
    
```

```{r echo=FALSE, include=FALSE}
df_graph <- melt(py$df_events, id=c("Year"))
```

```{r echo=FALSE, include=FALSE}
graph1 <- ggplot(df_graph, aes(x = Year, y = value, color = variable)) +
  geom_line()
