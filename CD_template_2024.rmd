...
params: 
  cd_state: "DE00"
  full_name: "Delaware's Congressional District"
  state: "DE"
  district: "00"
---
...
#  cd_state: "AK00"
#  full_name: "Alaska's Congressional District"
#  state: "AK"
#  district: "00"
#  cd_state: "AL05"
#  full_name: "Alabama's 5th Congressional District"
#  state: "AL"
#  district: "05"
...
knitr::opts_chunk$set(engine.path=list(
  python='/mnt/c/Users/steve/workspace/EDGI/EEW-ReportCard-Data/venv/bin/python'))
# library(reticulate)
# virtualenv_create('venv')
# py_install('pandas', envname='/mnt/c/Users/steve/workspace/EDGI/EEW-ReportCard-Data/venv')
# py_install('geopandas', envname='/mnt/c/Users/steve/workspace/EDGI/EEW-ReportCard-Data/venv')
# py_install('requests', envname='/mnt/c/Users/steve/workspace/EDGI/EEW-ReportCard-Data/venv')
# py_install('ECHO_modules', envname='/mnt/c/Users/steve/workspace/EDGI/EEW-ReportCard-Data/venv')

target_year_int <- strtoi(target_year)
u <- import('Region')
region <- u$Region(db='region_cds.db', type='Congressional District', 
      value=Region_CD, state=params$state, programs=c('CAA', 'CWA', 'RCRA'),
      base_year=target_year_int)
USAinspectionsper1000_All <- region$get_per_1000('inspections', 'USA')
inspectionsper1000_state <- region$get_per_1000('inspections', 'State')
inspectionsper1000_cd <- region$get_per_1000('inspections', 'CD')
USAviolationsper1000_All <- region$get_per_1000('violations', 'USA')
violationsper1000_state <- region$get_per_1000('violations', 'State')
violationsper1000_cd <- region$get_per_1000('violations', 'CD')
...
CAAenforcement_clean <- region$get_events('enforcements', 'CAA')
CWAenforcement_clean <- region$get_events('enforcements', 'CWA')
RCRAenforcement_clean <- region$get_events('enforcements', 'RCRA')
...
ranked_raw <- region$get_ranked()
ranked <- round(ranked_raw * 100.)
...
cwa_insp_years = create_df(db='region_cds.db', target_year=r.target_year_int,
               region_type='Congressional District', data_type='inspections',
               y_field='Count',program='CWA', states=[r.this_state,],
               region_selected=r.Region_CD)
cwa_viol_years = create_df(db='region_cds.db', target_year=r.target_year_int,
               region_type='Congressional District',data_type='violations',
               y_field='Count',program='CWA',states=[r.this_state,],
               region_selected=r.Region_CD)
cwa_enf_count_years = create_df(db='region_cds.db', target_year=r.target_year_int,
               region_type='Congressional District',data_type='enforcements',
               y_field='Count',program='CWA',states=[r.this_state,],
               region_selected=r.Region_CD)
cwa_enf_count_years = cwa_enf_count_years.drop('Amount', axis=1)
cwa_enf_dollar_years = create_df(db='region_cds.db', target_year=r.target_year_int,
               region_type='Congressional District',data_type='enforcements',
...
actionchange <- data.frame("PreBidenAverage" =mean(enforcement[1:20,3]),
                             "Biden Average" =mean(enforcement[21:23,3]))
...
if (CD == '00') {
  map_src <- paste('State_maps/map_',params$state,'.png',sep='')
} else {
  map_src <- paste('CD_maps/map_',params$state,CD,'.png',sep='')
}
```
...
cd_percent_bar <- ggplot(ranked_t, aes(fill=order, y=V1, x=Program, alpha=Program))+
  ...
  ggtitle(paste("Percentile Ranking Among All Districts, 2019-2023"))+
...
<h3>As an example, a Violations per Facility score of `r ranked$CWA_Viol_Pct` for CWA violations means that this district has more violations than `r ranked$CWA_Viol_Pct`% of all districts in the United States. From these scores we can assign letter grades to districts--the top 20%, those districts with more violations than 80% of all districts, would get an F; the districts scoring between 60% and 80% get a D; between 40% and 60% get a C; between 20% and 40% get a B; and less than 20% get an A. As such, `r params$full_name` receives the following grades:</h3>

<ul>
  <li><h3>CAA Violations per Facility - `r grade(ranked$CAA_Viol_Pct)`</h3></li>
  <li><h3>CAA Violations per Inspection - `r grade(ranked$CAA_Insp_Pct)`</h3></li>
  <li><h3>CAA Violations per Enforcement - `r grade(ranked$CAA_Enf_Pct)`</h3></li>
  <li><h3>CWA Violations per Facility - `r grade(ranked$CWA_Viol_Pct)`</h3></li>
  <li><h3>CWA Violations per Inspection - `r grade(ranked$CWA_Insp_Pct)`</h3></li>
  <li><h3>CWA Violations per Enforcement - `r grade(ranked$CWA_Enf_Pct)`</h3></li>
  <li><h3>RCRA Violations per Facility - `r grade(ranked$RCRA_Viol_Pct)`</h3></li>
  <li><h3>RCRA Violations per Inspection - `r grade(ranked$RCRA_Insp_Pct)`</h3></li>
  <li><h3>RCRA Violations per Enforcement - `r grade(ranked$RCRA_Enf_Pct)`</h3></li>
</ul>
...
<ul>
  ...
  <li>More <strong>enforcements</strong> when violations are found disincentivizes violating. Dividing violations by enforcements indicates EPA's willingness to penalize.</li>
</ul>
...
# Clean Water Act - Inspections, Violations, Enforcements Since 2001
...
<h3>Clean Water Act Violations*: <span style="color:#569b5e">Violations during the Trump administration `r vCWAchange[1,4]` `r vCWAchange[1,5]`</span> than the average over the previous 16 years, representing a <span style="color:#569b5e">`r vCWAchange[1,6]`% `r vCWAchange[1,7]`</span> in violations</h3>
...
effluentchange <- data.frame("Trump Average" =mean(effluentviolations[17:19,2]),
                             "Biden Average" =mean(effluentviolations[21:23,2])) # SH
...
effluentgraph <- ggplot(effluentviolations, mapping=aes(x=factor(Year), 
  ...
  scale_x_discrete(breaks=c(2004,2008,2012,2016,2020,2024))+
  theme_meg()+
...
# [17:20,2] Trump [17:19] first three years
# [21:23,2] Biden first three years
enforcementchange <- data.frame("Trump Average Count" =mean(enforcement[17:19,3]),
                               "Biden Average Count" =mean(enforcement[21:23,3]),
                               "Trump Average Fine" =mean(enforcement[17:19,2]),
                               "Biden Average Fine" =mean(enforcement[21:23,2])) # SH
...
enforcement <- merge(enforcement, inflation, by='Year', all.x=TRUE)
...
<h3>Comparing the first three years of the Trump administration to those of the Biden administration, there was a <span style="color:#569b5e">`r inspectionchange[1,5]`% `r inspectionchange[1,4]` in inspections,  `r enforcementchange[1,10]`%  `r enforcementchange[1,9]` in fines</span>, and a <span style="color:#569b5e">`r enforcementchange[1,8]`% `r enforcementchange[1,7]` in enforcement actions.</span></h3>
...
```{r echo=FALSE, include=FALSE, fig.height = 6, fig.width = 8, fig.align = "left"}
...
<font size="2">These figures show the ten facilities in this district with the worst environmental compliance based on the number of noncompliant quarters in the past 3 years (not necessarily consecutive). In some districts, for some programs, the number may be fewer than ten.
The EPA data shows records for the past 12 months for CAA and RCRA, and 13 months for CWA. The facilities shown with equal number of quarters in violation are selected at random. We only have room for ten in the graph, so we note when there are more facilities with the same number of quarters in violation.</font>
...
if (nrows_df > 0) {
  ...
  vCAAgraph <- ggplot(CAAviolations, mapping=aes(x=factor(Year), y=Count, fill=President, group=1))+
    ...
    theme(axis.text.x=element_text(size=10),
          axis.title.x = element_blank(), plot.subtitle = element_text(size = 10, color = "red", hjust=.5))
}
```
...
if (nrows_df > 0) {
  ...
  iCAAgraph <- ggplot(CAAinspections, mapping=aes(x=factor(Year), y=Count, fill=President, group=1))+
   ...
   labs(y=" # Inspections", x="Year", title="CAA Inspections\n", subtitle ="Potentially incomplete data")+
    ...
    theme_meg()+
    theme(axis.text.x=element_text(size=10),
          axis.title.x = element_blank(), plot.subtitle = element_text(size = 10, color = "dark gray", hjust=.5))
}
```
...
if (nrows_df > 0) {
  ...
  #adjust for inflation
  CAAenforcement <- merge(CAAenforcement, inflation, by='Year',  all.x=TRUE)
...
if (nrows_df > 0) {
  ...
  vCWAgraph <- ggplot(CWAviolations, mapping=aes(x=factor(Year), y=Count, fill=President, group=1))+
    ...
    theme_meg()+
    theme(axis.text.x=element_text(size=10),
          axis.title.x = element_blank(), plot.subtitle = element_text(color="darkgreen",hjust=.5))
}
```
...
if (nrows_df > 0) {
  ...
  #adjust for inflation
  CWAenforcement <- merge(CWAenforcement, inflation, by='Year', all.x=TRUE)
...
if (nrows_df > 0) {
  ...
  #adjust for inflation
  RCRAenforcement <- merge(RCRAenforcement, inflation, by='Year', all.x=TRUE)
...
### [OpenSecrets web page](`r leg_info$opensecrets_url`)
...
pdf_url <- paste("https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/",params$cd_state,"_",target_year,".pdf",sep="")
...
* ECHO does not record how many regulated facilities there were for programs in previous years. Therefore, we cannot calculate the number of inspections, enforcement actions, and violations per regulated facility before `target_year`.
...
### Notes on `target_year` data
We do not include data from `r strtoi(target_year)+1` because we are only part way through the year.
...
To enable direct comparison between changes in enforcement and violations over the years, we calculate the percent change in Clean Water Act violations and enforcement actions per district or state between Biden’s first three years in office, and the historical average in each district from 2001 to 2020. We analyze data since 2001, as EPA is most confident in its own data since 2001. We analyze violations data just for the Clean Water Act because that data is the most complete due to routine digital reporting requirements. We analyze all forms of enforcement actions, informal and formal. All data is drawn from the ECHO database. 
...
Trump and Biden Administration comparison: We compare levels of inspection and enforcement in the first three years of the Trump administration to those of the Biden administration. For these figures inspections and enforcement numbers for the CWA, CAA and RCRA are combined. 
...
EDGI’s work has been widely acknowledged, leading to EDGI testifying before Congress on declines in EPA enforcement, and hundreds of mentions in leading national and international media such as _The New York Times_,_The Washington Post_, Vice News, and CNN. For more about our work, read our [`r target_year` Annual Report](https://envirodatagov.org/publication/edgi-annual-report-`r target_year`/) and [`r target_year` Annual Report](https://envirodatagov.org/publication/edgi-annual-report-`r target_year`/). For more on EDGI see [our website](https://envirodatagov.org/).</font>
</div>
<a href="https://www.environmentalenforcementwatch.org/"><img src="logosandimages/eew_logo.png" style="float: left; width: 9%; margin-left: 30%"></a>
<a href="https://envirodatagov.org/"><img src="logosandimages/edgi_logo.png" style="float: right; width: 15%; margin-right: 30%" ></a>
