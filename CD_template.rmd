...
##### `r params$full_name`; seat held by `r leg_info$name` since `r leg_info$since_year` 
 <div class="container">
  ...
  <div class="bottom-left">Map shows congressional district or state with number of regulated facilities per program.</div>
  </div>
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State==params$cd_state, params$cd_state, "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State==params$cd_state, params$cd_state, "Other")
...
cd_state_dot <- CWAper1000[which(CWAper1000$CD.State==params$cd_state), ]
...
ggsave(paste("dotplot_",params$cd_state,".png",sep=""), dotplot, path=here(output_loc), scale = 2, width=8, height=5, units=c("cm"))

dotplot_loc <- paste(output_loc,"dotplot_",params$cd_state,".png",sep="")
cd_outliers <- ''
state_outliers <- ''
state_start <- FALSE
cd_start <- FALSE
...
<h3>In `r params$cd_state`, there were <span style="color:#569b5e">`r cd_state_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in `r target_year`.</h3>

<h2> Outliers (not shown):  Districts and states with more than 1200 violations per 100 installations.</h2>
...
```
# Highlights for  `r params$full_name`
...
### Notes on 2020 data
We do not include data from `r target_year` because we are only part way through the year. It is important to note, as well, that data from `r target_year` will be strongly influenced by the EPA’s decision to suspend, from March through August, pollution monitoring requirements for industries that claim to have been impacted by COVID-19. EDGI’s report on this policy [“More Permission to Pollute”](https://envirodatagov.org/more-permission-to-pollute-the-decline-of-epa-enforcement-and-industry-compliance-during-covid/) found that, despite relatively few facilities claiming the COVID exemption, a much larger proportion of facilities are still failing to report environmental data. 
